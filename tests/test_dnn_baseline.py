"""DNN baseline (torch MLP) contract tests — ticket 02, TDD."""
...
def test_create_model_reads_cfg_seed() -> None:
    from models.baselines.DNN.main import DNNModel

    m = DNNModel.from_config(_cfg(seed=7))
...
def test_fit_predict_hard_integer_labels() -> None:
    ...
    X, y = _toy_data()
    m = DNNModel.from_config(_cfg())
    m.fit(X, y)
    out = np.asarray(m.predict(X))
    assert out.ndim == 1
    assert np.issubdtype(out.dtype, np.integer)
    assert set(out.tolist()) <= {0, 1}
    # Pure read: repeated predict is identical.
...
def test_seed_changes_init() -> None:
    """Different cfg seeds must produce different initial weights."""
    import torch
    ...
    first_params = None
    for seed in (1, 2):
        m = DNNModel.from_config(_cfg(seed=seed))
        m.fit(X, y, X_val=X[:5], y_val=y[:5])
